export class UIController {
    ...
    updateWeightDisplay(weight) {
        if (this.weightDisplay) {
            this.weightDisplay.textContent = `Current Weight: ${weight.toFixed(1)} g`;
        }
    }
    ...
    getWeightDisplay() {
        if (this.weightDisplay) {
            ...
            return parseFloat(weightText.replace('Current Weight: ', '').replace(' g', ''));
        }
        return 0;
    }
    ...
    updateConnectButton(text) {
        if (this.connectButton) {
            ...
            if (text === 'Disconnect') {
                this.connectButton.classList.add('bg-red-300', 'text-white');
                this.connectButton.classList.remove('bg-purple-100','text-purple-800','border-purple-800', 'hover:bg-purple-200');
            } else {
                this.connectButton.classList.add('bg-purple-100','text-purple-800','border-purple-800', 'hover:bg-purple-200');
                this.connectButton.classList.remove('bg-red-300', 'text-white');
            }
        } else {
    ...
    renderCocktailList(cocktails, containerElement) {
        if (!cocktails.length) {
            containerElement.innerHTML = '<p class="text-gray-600 text-center text-lg col-span-full">No cocktails found.</p>';
            return;
        }
        containerElement.innerHTML = cocktails.map(cocktail => this.createCocktailCard(cocktail)).join('');
    }
    //random color for cocktail cards 
    getRandomColor() {
        let color;
        do {
            // Generates a random integer between 0 and 16777215 (FFFFFF in hex)
            color = Math.floor(Math.random() * 16777215).toString(16);
            // Pads with leading zeros to ensure it's always 6 characters long
            color = color.padStart(6, '0');
        } while (color.toUpperCase() === 'FFFFFF'); // Loop until a non-white color is generated
        return color;
    }
    createCocktailCard(cocktail) {
            const bgColor = this.getRandomColor();
            // const imageUrl = `https://placehold.co/300x200/${bgColor}/FFFFFF?text=${encodeURIComponent(cocktail.name)}`;
            const MAX_INGREDIENT_LENGTH = 70;
            const fullIngredients = cocktail.ingredients;
            let displayIngredients = fullIngredients;
            let initialClass = 'text-gray-600'; // Default color for full text

            // Check if ingredients text is longer than the maximum allowed length
            if (fullIngredients.length > MAX_INGREDIENT_LENGTH) {
                // Truncate the text for initial display
                displayIngredients = fullIngredients.substring(0, MAX_INGREDIENT_LENGTH) + '...';
                initialClass = 'text-gray-500 italic'; // Gray out and italicize truncated text
            }

            return `
              <a href="cocktail_step.html?id=${cocktail.id}" class="block">
                <div class="bg-white rounded-lg shadow-xl overflow-hidden transform transition-all duration-300 hover:scale-105 border border-purple-100">
                    <div class="w-full h-4 rounded-t-lg flex items-center justify-center" style="background-color: #${bgColor};">
                        <!-- No name text here, as per request -->
                    </div>
                    <div class="p-5">
                        <h3 class="text-2xl font-bold text-gray-900 mb-2">${cocktail.name}</h3>
                        <p
                            class="ingredients-toggle ${initialClass} text-sm cursor-pointer hover:text-purple-600 transition-colors duration-200"
                            data-cocktail-id="${cocktail.id}"
                            data-full-ingredients="${fullIngredients}"
                            data-truncated-ingredients="${fullIngredients.substring(0, MAX_INGREDIENT_LENGTH) + '...'}"
                            data-is-truncated="${fullIngredients.length > MAX_INGREDIENT_LENGTH ? 'true' : 'false'}"
    ...
    displayCocktails(cocktails, containerElement) {
        if (cocktails.length === 0) {
            containerElement.innerHTML = '<p class="text-gray-600 text-center text-lg col-span-full">No cocktails found matching your criteria.</p>';
            return;
        }
        containerElement.innerHTML = cocktails.map(cocktail => this.createCocktailCard(cocktail)).join('');
    }
    toggleIngredientsVisibility(event) {
        const target = event.target;

        // Check if the clicked element is an ingredients-toggle paragraph
        if (target.classList.contains('ingredients-toggle')) {
            const isTruncated = target.dataset.isTruncated === 'true';
            const fullIngredients = target.dataset.fullIngredients;
            const truncatedIngredients = target.dataset.truncatedIngredients;

            if (isTruncated) {
                // If currently truncated, show full text
                target.innerHTML = fullIngredients;
                target.dataset.isTruncated = 'false'; // Update state
                target.classList.remove('text-gray-500', 'italic');
                target.classList.add('text-gray-600');
            } else {
                // If currently full, show truncated text
                target.innerHTML = truncatedIngredients;
                target.dataset.isTruncated = 'true'; // Update state
                target.classList.remove('text-gray-600');
                target.classList.add('text-gray-500', 'italic');
            }
        }
    }
}
