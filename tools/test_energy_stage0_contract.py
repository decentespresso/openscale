...
class EnergyLightSleepContractTests(unittest.TestCase):
    ...
    def test_responsive_button_wake_profile(self):
        ...
        self.assertIn("storeEnergyLightSleepProfile(true, true, false, wasPlus)", toggle)
        self.assertIn("select(EnergyFeature::LightSleepPlus, true)", toggle)
        self.assertNotIn("if (!wasPlus)", toggle)
        self.assertNotIn('"Light Sleep+"', MENU)
        row = body(MENU, "inline void updateEnergyMenuRow(EnergyFeature feature)")
        self.assertIn("enabled ? 'x' : 'o'", row)
        self.assertNotIn("'+'", row)
        boost = body(FIRMWARE, "bool energyResponsiveButtonBoostActive(unsigned long now)")
        self.assertNotIn("lightSleepPlusActive()", boost)
        debounce = body(FIRMWARE, "void finishEnergyButtonWakeDebounce()")
        self.assertIn("if (energyPolicy.settings.lightSleepPlusActive()) return;", debounce)
    ...
    def test_light_sleep_transition_and_boot_fail_closed(self):
        ...
        enable_store = toggle.index("storeEnergyLightSleepProfile(true, true")
