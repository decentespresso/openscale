...
int main() {
  ...
  const uint32_t unansweredPingAt[] = { 500, 1000, 1500 };
  const uint32_t simulatedPlugExpiresAt = unansweredPingAt[2] + 2000;
  assert(!grinderReconnectDue(unansweredPingAt[2], GRINDER_RUNTIME_RECOVERY_DELAY_MS, simulatedPlugExpiresAt - 1));
  assert(grinderReconnectDue(unansweredPingAt[2], GRINDER_RUNTIME_RECOVERY_DELAY_MS, simulatedPlugExpiresAt + 250));
  GrinderRecoveryState recovery = grinderRecoveryStart();
  assert(grinderRecoveryCanRetryBusy(recovery));
  assert(grinderRecoveryBusyDelay(recovery) == GRINDER_RUNTIME_BUSY_BACKOFF_MS);
  recovery = grinderRecoveryRecordBusy(recovery);
  assert(recovery.active && recovery.fastBusyRetries == 1);
  assert(grinderRecoveryBusyDelay(recovery) == GRINDER_RUNTIME_BUSY_BACKOFF_MS);
  recovery = grinderRecoveryRecordBusy(recovery);
  assert(recovery.active && recovery.fastBusyRetries == GRINDER_RUNTIME_BUSY_FAST_RETRIES);
  assert(!grinderRecoveryCanRetryBusy(recovery));
  assert(grinderRecoveryBusyDelay(recovery) == GRINDER_RUNTIME_BUSY_RETRY_INTERVAL_MS);
  recovery = grinderRecoveryRecordBusy(recovery);
  assert(recovery.active && recovery.fastBusyRetries == GRINDER_RUNTIME_BUSY_FAST_RETRIES);
  assert(grinderRecoveryBusyDelay(recovery) == GRINDER_RUNTIME_BUSY_RETRY_INTERVAL_MS);
  recovery = grinderRecoveryComplete();
  assert(!recovery.active && recovery.fastBusyRetries == 0);
  assert(!grinderRecoveryCanRetryBusy(recovery));
