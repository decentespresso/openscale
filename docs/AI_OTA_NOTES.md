...
Only one start request exists at a time. Every transport admits through `remoteQueueWifiUpdate()`, USB included; none of them call `wifiUpdate()` directly. It refuses a request while `WSP_WIFI_UPDATE` is queued, while `pendingOtaDispatching` is set, and while `b_ota` or `b_pullOtaRunning` is set, all read in one critical section, so a queued request cannot be overtaken by a later start on another transport. A request that reaches dispatch while an update is already running is dropped and logged rather than requeued, so an accepted request can never fire after an unrelated update ends. `processWsPendingCmds()` sets that flag in the same critical section that clears the pending bit and calls `remoteFinishWifiUpdateDispatch()` only after `wifiUpdate()` has marked the update running, so acceptance stays atomic across the handoff. A request deferred back to the queue clears the flag with the bit.
