...
`v3.1.14-preview.4` is also allowed and reports `3.1.14-preview.4-custom` in custom builds. Unlike the non-custom preview.3 release, official preview.4 publishes its own signed firmware/LittleFS recovery manifest. Do not generalize the preview.3 recovery limitation to preview.4 or hosted custom builds.

Add the prospective `vX.Y.Z` tag to `FIRMWARE_REFS`, compatible plugin manifests, and the Worker's `ALLOWED_FIRMWARE_REFS`, then regenerate both custom-build catalogs. Keep the live configurator on its current default until the real tag exists. When the tag does not exist, pull-request jobs may create a temporary local tag at the candidate commit and must pass its resolved commit explicitly when compiling the stable custom build. Do not create or push the real tag before the pre-tag gate passes.

The 3.1.14 cutover configurator offers only `3.1.14 (stable)` and `main (development)`, with stable selected by default. Main remains available for testing and requires confirmation before each build request, including retries. Preview selection links are rejected; existing saved builds and recovery assets remain available. Keep preview refs in the service catalogs and Worker allow-list for compatibility rather than deleting recovery support to hide a dropdown option.

Keep the cutover UI changes in a draft PR until the catalog commit reaches the trusted builder branch, the real stable tag exists, and tagged custom builds pass verification. Deploy the Worker support before deploying the stable-default configurator. Retain `main` in the catalog and Worker allow-list; development builds still consume the shared build quota. The resulting stable custom firmware reports `3.1.14-custom`.
