...
Custom builds set `SOURCE_DATE_EPOCH` from that commit so compiler date macros do not make the cached artifact depend on the day it was requested.

## Artifacts

A successful combination stores these objects under its hash:
...
`HDS_FW_custom.zip` is the only user-facing download. It contains exactly `firmware.bin`, `bootloader.bin`, `partitions.bin`, and `littlefs.bin`. `firmware.factory.bin` is neither listed nor published. The manifest and dependency inventory remain service metadata; the manifest records the size and SHA-256 of the archive and each binary inside it.
...
The workflow uploads the archive and `dependencies.txt` first and uploads `build-manifest.json` last. The manifest is the commit marker: a combination is `ready` only when it exists and the published object sizes and SHA-256 values match. A request after expiry is a normal cache miss and may rebuild the same combination.

As checked on 2026-08-08, the monthly free allowance is 10 GB-month of storage, 1 million Class A operations, 10 million Class B operations, and free egress. A current three-object build entry is about 3.22 MB, so storage reaches the paid tier at roughly 3,000 retained combinations before operation limits become relevant. The 5 GiB single-part upload limit and 5 TiB object limit leave ample room for every build artifact.
...
- The matching build manifest records every binary in the ZIP.
