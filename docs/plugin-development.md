...
Presentation content is not included in firmware build identity. Runtime app files, effective asset paths and hashes, and the app label are included. Plugin authors should provide uncompressed HTML, CSS, JavaScript, and SVG. Do not supply `.html.gz`, `.js.gz`, `.css.gz`, or `.svg.gz` assets in either `webapp/` or the manifest; the filesystem build produces deterministic gzip variants and rejects those precompressed plugin files.
