...
- **`setSamplesInUse()` clears the ring buffer and resets `validSamples=0`.** Right after a sensitivity change (`ble.h`, `usbcomm.h`, `menu.h`, and `setup()`'s `setSamplesInUse(1)`), `getData()` returns `0.0` until the window refills, then ramps up. The old vendored lib backfilled with the last smoothed value, so this transient blip-to-zero on live sensitivity switches is new. `start()` pre-fills via its settle loop, so boot-time tare is fine.
  - **Don't capture a reading until the smoothed value has plateaued** — and a load step (placing the calibration weight) restarts the ramp just like a `setSamplesInUse()` change does. Calibration learned this the hard way (PR #71): `calibrationCaptureRaw()` in `menu.h` used to grab the first reads where `validSamples>=16`, which is mid-ramp, so the load delta came in a fraction of its settled value with a huge read-to-read spread and tripped the `unstable_load` gate on hardware that weighs perfectly. The fix polls until the read-to-read change stays under the stability limit for several consecutive reads before measuring. Any new code that reads `getData()`/`smoothedValue` for a one-shot measurement after a window change or load step must wait out the ramp the same way.
